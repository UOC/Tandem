...
$linkedTasks = $gestorBD->getAvailableTasks($exerciseId, $course_id);
?>
<table class="table">
    <thead>
    <tr>
    ...
    <?php foreach($linkedTasks as $task) { ?>
        <tr>
            ...
            <td><?php echo $LanguageInstance->get($task['enabled']?'yes':'no')?></td>
            <td>
                <a onclick="linkTask(<?php echo $task['id']?>)" class="tandem-btn">+</a>
            </td>
        </tr>
    <?php }
