...
if(empty($id_tandem) or empty($id_user) or empty($task_number) or empty($mood) or empty ($comment)){
	echo json_encode(array("result" => "Missing parameter"));
}

$gestordb = new GestorBD();
$response = $gestordb->setTaskEvaluation($id_tandem, $id_user, $task_number, $mood, $comment);
