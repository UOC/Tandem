...
class TandemPDF extends TCPDF {
     ...
     * @param bool $pdfa
     * @param bool $image_file
     * @param Language|false $LanguageInstance
     * @param string $username
     */
    public function __construct($orientation = 'P', $unit = 'mm', $format = 'A4', $unicode = true,
                                $encoding = 'UTF-8', $diskcache = false, $pdfa = false, $image_file = false,
                                $LanguageInstance = false, $username = '') {
...
function generatePDF($user_id, $course_id) {
    ...
    $feedBacks = $gestorBD->getAllUserFeedbacks($user_id, $course_id, -1, -1, '', '', $_SESSION[USE_WAITING_ROOM_NO_TEAMS]);
    ...
    $pdf = new TandemPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false, false, __DIR__ . '/tcpdf_min/logo_Tandem.png', $LanguageInstance, $username);
    ...
    if (!empty($feedBacks)) {
        foreach ($feedBacks as $key => $value) {
            if (!empty($value['feedback_form'])) {
                if (!$show_tandem && !$_SESSION[USE_WAITING_ROOM_NO_TEAMS]) {
...
//                            '<b>' . $LanguageInstance->get('Overall Grade') . ': </b>' . getSkillsLevel(getOverallAsIdentifier($user_data['overall_grade']), $LanguageInstance) . '<br>	';
                ...
                $title = str_replace(array('%1', '%2'), array($value['created'], $value['total_time']),
                    $LanguageInstance->get('Tandem session created %1 with a total duration of %2'));
                $html .= '<div class="tit"><b>' . $title . '</b>';
                $html .= '<p class="tit">' . strtoupper($LanguageInstance->get('Sent Feedback')) . '</p>';
                if ($_SESSION[USE_WAITING_ROOM_NO_TEAMS]) {
                    $html .= '<b>' . $LanguageInstance->get('Grammatical Resource') . ': </b>' . $value['feedback_form']->grammaticalresource . '%<br>
								<b>' . $LanguageInstance->get('Lexical Resource') . ': </b>' . $value['feedback_form']->lexicalresource . '%<br>
