...
//Tornem a l'index
	...
	if(!empty($selectedUser) && $user_obj->instructor == 1){
		...
		$feedbacks = $gestorBD->getAllUserFeedbacks($user_obj->id,$course_id, $showFeedback, $finishedTandem, '', '', $_SESSION[USE_WAITING_ROOM_NO_TEAMS]);	
	}
}
...
<script>
	$(document).ready(function(){
		...
		<?php } ?>
		<?php if(!$_SESSION[USE_WAITING_ROOM_NO_TEAMS] && !$firstProfileForm && $user_obj->instructor != 1){ ?>
			$("#registry-modal-form").modal("show");
		<?php } ?>
		<?php if ($_SESSION[USE_WAITING_ROOM_NO_TEAMS] && !$newProfileForm && $user_obj->instructor != 1) { ?>
	...
	<div class='row'>
  	...
  	<div class="row">
   	...
   	<div class="row well">
        ...
        <?php
        }else if(!empty($firstProfileForm)){
            ?>
   	<div class="row well">
   		<div class="col-md-6">
   			<div class="list_group">
...
<!-- Modal -->
<div class="modal fade bs-example-modal-lg" id="registry-modal-form-new" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	...
	<form  id='extra-info-new' role="form" method="POST">
  	<h4><?php echo $LanguageInstance->get('During the course I want to improve my');?></h4>
  	  	<div class="form-group">
	   		...
	   		<input type='text' name='grammaticalresource' class='slider'  data-slider-id="ex1Slider" data-slider-value='<?php echo isset($newProfileForm['data']->grammaticalresource) ? $newProfileForm['data']->grammaticalresource : '' ?>' /> %
  		</div>
  		<div class="form-group">
	   		<label for="input2">
	   			<?php echo $LanguageInstance->get('Lexical Resource');?>&nbsp;&nbsp;
	   		</label>
	   		<input type='text' name='lexicalresource' class='slider' data-slider-id="ex2Slider" value="" data-slider-value='<?php echo isset($newProfileForm['data']->lexicalresource) ? $newProfileForm['data']->lexicalresource : '' ?>'  /> %
  		</div>
  		<div class="form-group">
	   		<label for="input2">
	   			<?php echo $LanguageInstance->get('Discourse Mangement');?>&nbsp;&nbsp;
	   		</label>
	   		<input type='text' name='discoursemangement' class='slider' data-slider-id="ex3Slider" value="" data-slider-value='<?php echo isset($newProfileForm['data']->discoursemangement) ? $newProfileForm['data']->discoursemangement : '' ?>'  /> %
  		</div>
  		<div class="form-group">
	   		<label for="input2">
	   			<?php echo $LanguageInstance->get('Pronunciation');?>&nbsp;&nbsp;
	   		</label>
	   		<input type='text' name='pronunciation' class='slider' data-slider-id="ex4Slider" value="" data-slider-value='<?php echo isset($newProfileForm['data']->pronunciation) ? $newProfileForm['data']->pronunciation : '' ?>'  /> %
  		</div>
  		<div class="form-group">
	   		<label for="input2">
	   			<?php echo $LanguageInstance->get('Interactive Communication');?>&nbsp;&nbsp;
	   		</label>
	   		<input type='text' name='interactivecommunication' class='slider' data-slider-id="ex5Slider" value="" data-slider-value='<?php echo isset($newProfileForm['data']->interactivecommunication) ? $newProfileForm['data']->interactivecommunication : '' ?>'  /> %
  		</div>
      ...
      <!--span class="small"><?php echo $LanguageInstance->get('cannot_be_modified')?></span-->
        <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo $LanguageInstance->get('Close');?></button>
        <button type="button" id='submit-extra-info-new' class="btn btn-success"><?php echo $LanguageInstance->get('Save changes');?></button>
      </div>
    </div>
  </div>
</div>
