		...
		<!-- main-container -->
  		<div id="main-container">
			...
			<div id="main">
				...
				<h4><a href="selectUserAndRoom.php">&larr;&nbsp;<?php echo $LanguageInstance->get('back')?></a></h4>
				<h1><?php echo $LanguageInstance->get('mange_exercises_tandem')?></h1>
				<form enctype="multipart/form-data" method="post" action="">
					<p><label><?php echo $LanguageInstance->get('exercise_name')?>: <input type="text" name="name" value="<?php echo $exercise_name ?>"/></label></p>
					<p><label><?php echo $LanguageInstance->get('choose_zip_file')?>: <input type="file" name="zip_file" /> Max. <?php echo $upload_mb; ?> MB</label></p>
					<p><label><?php echo $LanguageInstance->get('overrides_xml_file')?>: <select name="overrides_xml_file" ><option value="0"><?php echo $LanguageInstance->get('no')?></option><option value="1" <?php echo $exercise_id>0?'selected="selected"':''?>><?php echo $LanguageInstance->get('yes')?></option></select> </label></p>
					...
					<div id="tableContainer" class="tableContainer">
						...
						<tbody class="scrollContent">
							...
							<tr class="<?php echo $i%2==0?'normalRow':'alternateRow'?>">
								...
								<td><a href="manage_exercises_tandem.php?enabled=<?php echo $exercise['id']?>&action=<?php echo $exercise['enabled']?>"><?php echo $LanguageInstance->get($exercise['enabled']=='1'?'yes':'no')?></a></td>
								<td><a href="manage_exercises_tandem.php?update_exercise_form_id=<?php echo $exercise['id']?>"><?php echo $LanguageInstance->get('update')?></a></td>
								<td><a href="manage_exercises_tandem.php?delete=<?php echo $exercise['id']?>"><?php echo $LanguageInstance->get('delete')?></a></td>
							</tr>
							<?php 
							$i++;
							}?>
